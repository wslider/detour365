...
async function loadLatestVideo() {
  ...
  try {
    ...
    if (latestVideoLink) {
      latestVideoLink.href = `https://www.youtube.com/embed/${videoId}`;
      console.log('Latest video link loaded successfully:', videoId);
    } else {
      console.error('Link element not found in the DOM');
    }

  } catch (error) {
    console.error('Failed to load latest video Link:', error.message);
    }
