...
export async function generateShareableVerseImage() {
    ...
    const currentReference = document.getElementById('verseReference').innerText.trim();if (!currentVerseText || currentVerseText.includes("Loading")) {
...
try {
    const bg = document.getElementById('share-verse-bg');
    if (bg && !bg.complete) {
